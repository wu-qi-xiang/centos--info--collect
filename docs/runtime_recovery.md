...
- `restore_preflight.py` 只接受明确指定的、已存在且为空的隔离目录。它绝不替换源数据库、源 `uploads/` 或应用运行目录。
...
python3 scripts/backup_local.py \
  ...
  --output /var/backups/pylinux/pylinux-20260720.tar.gz
```
...
先创建专用、空的目录。这个目录不应是项目目录、生产数据目录或归档所在目录。预检会先拒绝不安全或损坏的归档，验证所有数据成员的 SHA-256 校验和，再把数据库和上传文件写入该隔离目录，并对恢复后的 SQLite 文件执行只读 `PRAGMA integrity_check`。

```bash
mkdir -p /var/tmp/pylinux-restore-drill
python3 scripts/restore_preflight.py \
  --archive /var/backups/pylinux/pylinux-20260720.tar.gz \
  --target-dir /var/tmp/pylinux-restore-drill
```

成功时会输出 JSON 报告，并在隔离目录中生成：
...
预检不会运行 Django 迁移、启动服务、登录应用或连接 SSH、SMTP、Webhook 等外部系统。需要进一步演练时，基于这份已验证副本创建单独的测试应用环境，使用无生产凭据的配置运行 `manage.py check` 和受控的业务验证。
