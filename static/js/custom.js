    ...
    async testConnection(serverId) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // 更新状态指示器
                serverStatusManager.updateStatusIndicator(serverId, 'online');
            } else {
                this.showNotification(`Connection test failed: ${data.error}`, 'error');
                serverStatusManager.updateStatusIndicator(serverId, 'offline');
            }
        } catch (error) {
...
function copyToClipboard(text) {
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(text).then(() => {
            connectionTester.showNotification(`Copied: ${text}`, 'success');
        }).catch(err => {
...
function fallbackCopyTextToClipboard(text) {
    ...
    try {
        ...
        if (successful) {
            ...
            connectionTester.showNotification('Failed to copy text', 'error');
        }
    } catch (err) {
...
function refreshServers() {
    ...
    // 强制更新所有服务器状态
    serverStatusManager.updateAllServerStatus().then(() => {
        connectionTester.showNotification('Server list refreshed!', 'success');
    }).catch(error => {
        console.error('Refresh failed:', error);
        connectionTester.showNotification('Failed to refresh server list', 'error');
    }).finally(() => {
        // 恢复按钮状态
        setTimeout(() => {
...
function confirmDelete(serverId, serverName) {
    if (confirm(`Are you sure you want to delete server "${serverName}"?`)) {
        window.location.href = `/linux/delete/${serverId}/`;
    }
}

// 连接测试函数
function testConnection(serverId) {
    connectionTester.testConnection(serverId);
}
