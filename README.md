...
监控对接支持配置多个带名称的 Prometheus 实例、Alertmanager 基础 URL 和连接测试，已配置的对接会展示在监控管理页面。仅运维员可以创建、更新、删除或测试对接；“指标查询”使用首个已启用的 Prometheus 执行即时 PromQL，并将向量、矩阵、标量或字符串结果统一展示为表格。单次响应最多展示 500 行，同时保留完整结果行数用于提示。
